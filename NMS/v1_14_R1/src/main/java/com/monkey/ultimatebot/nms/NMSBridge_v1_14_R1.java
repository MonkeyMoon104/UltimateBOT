...
import com.monkey.ultimatebot.bot.ai.TrainingBotHandle_v1_14_R1;
import com.monkey.ultimatebot.bot.ai.TrainingBot_v1_14_R1;
import com.monkey.ultimatebot.bot.ai.fakeplayer.v1_14_R1.VersionedItemActionContext;
import com.monkey.ultimatebot.bot.ai.fakeplayer.v1_14_R1.VersionedTntMinecart;
...
public final class NMSBridge_v1_14_R1 implements INMSBridge {
    ...
    public ITrainingBot createTrainingBot(
        ...
        TrainingBot_v1_14_R1 entity = new TrainingBot_v1_14_R1(
                server,
                world,
    ...
    public void moveBot(Player bot, double x, double y, double z) {
        ...
        ITrainingBot handle = trainingHandle(nativeBot);
        if (handle != null) {
            respawnPacketViewers(handle);
        } else {
            broadcastBotPosition(bot);
        }
    }

    private static @Nullable ITrainingBot trainingHandle(EntityPlayer nativeBot) {
        if (nativeBot instanceof TrainingBot_v1_14_R1) {
            return ((TrainingBot_v1_14_R1) nativeBot).handle();
        }
        return null;
    }

    private void respawnPacketViewers(ITrainingBot bot) {
        EntityPlayer nativeBot = nativeBot(bot);
        PacketPlayOutEntityDestroy destroy = new PacketPlayOutEntityDestroy(nativeBot.getId());
        Location botLoc = bot.asBukkitPlayer().getLocation();
        Map<EquipmentSlotKind, ItemStack> equipment = currentEquipment(bot);
        UltimateBot plugin = bot.getPlugin();
        double maxDistSq = 64.0D * 64.0D;
        for (Player online : Bukkit.getOnlinePlayers()) {
            if (online.getEntityId() == nativeBot.getId()) {
                continue;
            }
            if (online.getWorld() != botLoc.getWorld()) {
                continue;
            }
            ...
            sendTabListAdd(online, bot);
            sendSpawnAndMeta(online, bot);
            sendEquipment(online, bot, equipment);
            if (plugin == null) {
                continue;
            }
            final Player viewer = online;
            Bukkit.getScheduler()
                    .runTaskLater(
                            plugin,
                            () -> {
                                if (viewer.isOnline() && !bot.isRemoved()) {
                                    sendTabListRemove(viewer, bot);
                                }
                            },
                            2L);
        }
    ...
    private Map<EquipmentSlotKind, ItemStack> currentEquipment(ITrainingBot bot) {
        Map<EquipmentSlotKind, ItemStack> equipment =
                new java.util.EnumMap<EquipmentSlotKind, ItemStack>(EquipmentSlotKind.class);
        EquipmentSlotKind[] slots = EquipmentSlotKind.values();
        for (int i = 0; i < slots.length; i++) {
            EquipmentSlotKind slot = slots[i];
            if (toNmsSlot(slot) == null) {
                continue;
            }
            equipment.put(slot, getBotItem(bot, slot));
        }
        return equipment;
    }
    ...
    public void explode(Location location, @Nullable Player cause, float power, boolean blockDamage) {
        WorldServer world =
                ((CraftWorld) java.util.Objects.requireNonNull(location.getWorld(), "location world")).getHandle();
    ...
    public boolean useItemOnBlock(
        ...
        net.minecraft.server.v1_14_R1.ItemStack requested = CraftItemStack.asNMSCopy(stack);
        if (requested.isEmpty()) {
            return false;
        }
        net.minecraft.server.v1_14_R1.ItemStack held =
                enumHand == EnumHand.OFF_HAND ? nativeBot.getItemInOffHand() : nativeBot.getItemInMainHand();

        if (held.isEmpty() || held.getItem() != requested.getItem()) {
            setHeldSilently(nativeBot, enumHand, requested);
            held = enumHand == EnumHand.OFF_HAND ? nativeBot.getItemInOffHand() : nativeBot.getItemInMainHand();
        }
        if (held.isEmpty()) {
            return false;
        }
    ...
    private static boolean placeEndCrystalDirect(WorldServer world, EntityPlayer placer, BlockPosition clicked) {
        net.minecraft.server.v1_14_R1.IBlockData data = world.getType(clicked);
    ...
    private static void setHeldSilently(
            EntityPlayer nativeBot, EnumHand hand, net.minecraft.server.v1_14_R1.ItemStack stack) {
    ...
    private static void restoreHeldCount(net.minecraft.server.v1_14_R1.ItemStack nmsStack, int previousCount) {
    ...
    public void sendSpawnAndMeta(Player viewer, ITrainingBot bot) {
        ...
        handle.playerConnection.sendPacket(
                new PacketPlayOutEntity.PacketPlayOutEntityLook(nativeBot.getId(), yaw, pitch, nativeBot.onGround));
        handle.playerConnection.sendPacket(
    ...
    public void sendEquipment(Player viewer, ITrainingBot bot, Map<EquipmentSlotKind, ItemStack> equipment) {
        ...
        for (Map.Entry<EquipmentSlotKind, ItemStack> entry : equipment.entrySet()) {
            ...
            handle.playerConnection.sendPacket(
                    new PacketPlayOutEntityEquipment(entityId, slot, CraftItemStack.asNMSCopy(entry.getValue())));
        }
    }
    ...
    public void setBotSwimming(ITrainingBot bot, boolean swimming) {
        EntityPlayer nativeBot = nativeBot(bot);
        if (nativeBot instanceof TrainingBot_v1_14_R1) {
            ((TrainingBot_v1_14_R1) nativeBot).applySwimState(swimming);
        } else {
    ...
    public void setBotItem(ITrainingBot bot, EquipmentSlotKind slot, @Nullable ItemStack stack) {
        ...
        net.minecraft.server.v1_14_R1.ItemStack nms = CraftItemStack.asNMSCopy(resolved);

        if (nmsSlot == EnumItemSlot.MAINHAND || nmsSlot == EnumItemSlot.OFFHAND) {
            setHeldSilently(
                    nativeBot(bot), nmsSlot == EnumItemSlot.OFFHAND ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, nms);
            return;
        }
    ...
    public void setBotVelocity(ITrainingBot bot, Vector velocity) {
        ...
        nativeBot.setMot(Double.isFinite(x) ? x : 0.0D, Double.isFinite(y) ? y : 0.0D, Double.isFinite(z) ? z : 0.0D);
    ...
    private static void applyHandItemAttributes(EntityPlayer nativeBot, EnumItemSlot slot) {
        net.minecraft.server.v1_14_R1.ItemStack held = nativeBot.getEquipment(slot);
        if (held == null) {
            return;
        }
    ...
    public Entity spawnExplosiveMinecart(Location location) {
        WorldServer world = ((CraftWorld) java.util.Objects.requireNonNull(location.getWorld(), "world")).getHandle();
        VersionedTntMinecart cart = new VersionedTntMinecart(world, location.getX(), location.getY(), location.getZ());
        cart.dead = false;
    ...
    public void setExplosiveMinecartFuseTicks(Entity minecart, int ticks) {
        ...
        net.minecraft.server.v1_14_R1.Entity handle = ((CraftEntity) minecart).getHandle();
    ...
    private static EntityPlayer nativeBot(ITrainingBot bot) {
        if (bot instanceof TrainingBotHandle_v1_14_R1) {
            return ((TrainingBotHandle_v1_14_R1) bot).nativeEntity();
        }
