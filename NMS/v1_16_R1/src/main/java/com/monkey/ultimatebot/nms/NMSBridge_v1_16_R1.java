...
import com.monkey.ultimatebot.bot.ai.TrainingBotHandle_v1_16_R1;
import com.monkey.ultimatebot.bot.ai.TrainingBot_v1_16_R1;
import com.monkey.ultimatebot.bot.ai.fakeplayer.v1_16_R1.VersionedItemActionContext;
import com.monkey.ultimatebot.bot.ai.fakeplayer.v1_16_R1.VersionedTntMinecart;
...
public final class NMSBridge_v1_16_R1 implements INMSBridge {
    ...
    public ITrainingBot createTrainingBot(
        ...
        TrainingBot_v1_16_R1 entity = new TrainingBot_v1_16_R1(
                server,
                world,
    ...
    public boolean useItemOnBlock(
        ...
        net.minecraft.server.v1_16_R1.ItemStack requested = CraftItemStack.asNMSCopy(stack);
        if (requested.isEmpty()) {
            return false;
        }
        net.minecraft.server.v1_16_R1.ItemStack held =
    ...
    private static void setHeldSilently(
            EntityPlayer nativeBot, EnumHand hand, net.minecraft.server.v1_16_R1.ItemStack stack) {
    ...
    private static void restoreHeldCount(net.minecraft.server.v1_16_R1.ItemStack nmsStack, int previousCount) {
    ...
    public void throwEnderpearl(Player bot, Vector targetPos) {
        ...
        double scale = distance < 15 ? Math.min(distance * 0.08, 1.2) : Math.min(distance * 0.06, 1.8);
    ...
    public void sendSpawnAndMeta(Player viewer, ITrainingBot bot) {
        ...
        handle.playerConnection.sendPacket(
                new PacketPlayOutEntity.PacketPlayOutEntityLook(nativeBot.getId(), yaw, pitch, nativeBot.isOnGround()));
        handle.playerConnection.sendPacket(
    ...
    public void setBotItem(ITrainingBot bot, EquipmentSlotKind slot, @Nullable ItemStack stack) {
        ...
        net.minecraft.server.v1_16_R1.ItemStack nms = CraftItemStack.asNMSCopy(resolved);
    ...
    public void setBotOnGround(ITrainingBot bot, boolean onGround) {
        EntityPlayer nativeBot = nativeBot(bot);
        if (nativeBot instanceof TrainingBot_v1_16_R1) {
            ((TrainingBot_v1_16_R1) nativeBot).setOnGroundFlag(onGround);
        }
    }
    ...
    private static void applyHandItemAttributes(EntityPlayer nativeBot, EnumItemSlot slot) {
        net.minecraft.server.v1_16_R1.ItemStack held = nativeBot.getEquipment(slot);
        if (held == null) {
            return;
        }
    ...
    public void setExplosiveMinecartFuseTicks(Entity minecart, int ticks) {
        ...
        net.minecraft.server.v1_16_R1.Entity handle = ((CraftEntity) minecart).getHandle();
    ...
    private static EntityPlayer nativeBot(ITrainingBot bot) {
        if (bot instanceof TrainingBotHandle_v1_16_R1) {
            return ((TrainingBotHandle_v1_16_R1) bot).nativeEntity();
        }
