...
public class TargetsTab {
    ...
    private class ScrollUpItem extends AbstractScrollGuiBoundItem {
        ...
        public ItemProvider getItemProvider(Player viewer) {
            ...
            if (gui.getLine() <= 0) {
                String cantScroll = context.getTraining().getLangString("gui.targets-tab.scroll-up.cant-scroll", "&7Sei gi� in cima");
    ...
    private class ScrollDownItem extends AbstractScrollGuiBoundItem {
        ...
        public ItemProvider getItemProvider(Player viewer) {
            ScrollGui<?> gui = getGui();
            String name = context.getTraining().getLangString("gui.targets-tab.scroll-down.name", "&aScorri gi�");
            ItemBuilder builder = new ItemBuilder(Material.ARROW)
                    .setLegacyName(ChatColorUtils.translate(name));
            if (gui.getLine() >= gui.getMaxLine()) {
                String cantScroll = context.getTraining().getLangString("gui.targets-tab.scroll-down.cant-scroll", "&7Sei gi� in fondo");
                builder.addLegacyLoreLines(ChatColorUtils.translate(cantScroll));
            }
            return builder;
        }
