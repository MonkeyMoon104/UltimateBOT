...
/**
 * Arm-swing animation across Paper versions that share a Craft revision.
 *
 * <p>{@link LivingEntity#swingMainHand()} exists from 1.15.2 and already reaches nearby clients on
 * this Craft revision. 1.15 / 1.15.1 lack the Bukkit method: call NMS {@code EntityLiving#a(EnumHand)}
 * and send {@code PacketPlayOutAnimation} from the NMS {@code Entity} package — not from
 * {@code TrainingBot}'s plugin package.
 */
    ...
    private static final class NmsSwing {
        ...
        static NmsSwing resolve(Player sample) {
            try {
                ...
                Constructor<?> ctor =
                        packetClass == null ? null : findAnimationCtor(packetClass, handle.getClass());
            ...
            for (Method method : handleClass.getMethods()) {
                ...
                if (params.length == 1) {
                    if ("swingHand".equals(method.getName())) {
                        ...
                        && "swingHand".equals(method.getName())) {
                    twoArg = method;
                }
            }
            if (named != null) {
