...
/**
 * Melee attack across Paper versions that share a Craft revision.
 *
 * <p>{@link Player#attack(Entity)} exists from 1.15.2. Paper 1.15.1 is still Craft {@code v1_15_R1},
 * so this is dual-path in core — not a new NMS package. Crystals are not {@code LivingEntity}, so
 * CPvP must not hard-link the Bukkit method. Fallback is NMS {@code EntityHuman#attack}.
 */
public final class PlayerAttackAccess {
    ...
    public static void attack(Player player, Entity target) {
        ...
        try {
            ...
            // CraftPlayer on 1.15.1 has no attack(); NMS EntityHuman does.
        }
