...
/**
 * Attack-strength scale across Paper versions that share a Craft revision.
 *
 * <p>{@link HumanEntity#getAttackCooldown()} exists from 1.15.2. Paper 1.15.1 is still Craft
 * {@code v1_15_R1}, so this is dual-path in core — not a new NMS package. Fallback is NMS
 * {@code EntityHuman#getAttackCooldown(float)} / {@code s(float)}.
 */
    ...
    private static final class NmsCooldown {
        ...
        float get(Player player, float advance) {
            ...
            try {
                ...
                // fully charged is the safer axe/shield default
            }
            return 1.0F;
        }
