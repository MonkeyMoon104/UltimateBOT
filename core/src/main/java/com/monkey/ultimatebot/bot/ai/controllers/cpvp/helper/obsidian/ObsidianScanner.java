        ...
        for (int y = -3; y <= 2; y++) {
            for (int x = -6; x <= 6; x++) {
                for (int z = -6; z <= 6; z++) {
                    ...
                    Material type = world.getBlockAt(checkPos.getBlockX(), checkPos.getBlockY(), checkPos.getBlockZ())
                            .getType();
                    if (type == Material.OBSIDIAN || type == Material.BEDROCK) {
                        if (!BlockPassableAccess.isPassable(world.getBlockAt(
                                        checkPos.getBlockX(), checkPos.getBlockY() + 1, checkPos.getBlockZ()))
                                || !BlockPassableAccess.isPassable(world.getBlockAt(
                                        checkPos.getBlockX(), checkPos.getBlockY() + 2, checkPos.getBlockZ()))) {
                            continue;
                        }
                        org.bukkit.Location center = new org.bukkit.Location(
                                world,
                                checkPos.getBlockX() + 0.5D,
                                checkPos.getBlockY() + 0.5D,
                                checkPos.getBlockZ() + 0.5D);
