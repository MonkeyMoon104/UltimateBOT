...
class CobwebCombatAwarenessTest {
    ...
    void detectsMovementNearTheWebBoundary() {
        assertThat(CobwebCombatAwareness.nearlyExiting(
                        new Vector(10.9D, 64.0D, 20.5D), new Vector(0.03D, 0.0D, 0.0D)))
                .isTrue();
    }
    ...
    void keepsStationaryAndCenteredTargetsContained() {
        assertThat(CobwebCombatAwareness.nearlyExiting(
                        new Vector(10.5D, 64.0D, 20.5D), new Vector(0.03D, 0.0D, 0.0D)))
                .isFalse();
