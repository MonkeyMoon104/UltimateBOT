# MinecraftBot Core Module

## Overview
`mcbot-core` is the runtime implementation module of the MinecraftBot project.
...
- Public API publication through `MinecraftBotAPI` and `MinecraftBotReadyEvent`
...
### Bootstrap
`com.monkey.mcbot.MinecraftBot` is the plugin entrypoint. During `onEnable()` it:
...
- registers `MinecraftBotAPI`
- fires `MinecraftBotReadyEvent`
...
| --- | --- | --- |
| `/bot` | Open the normal bot GUI | `mcb.bot.use` |
| `/botally` | Open the ally bot GUI | `mcb.bot.use` |
| `/botevent` | Open the event bot GUI | `admin.host.bot` |
| `/botteamally` | Open the TEAM_ALLY GUI for an existing team member | `mcb.bot.use` |
| `/botteamally create <owner1> <owner2> [owner3 ...]` | Prepare a shared TEAM_ALLY bot for multiple online owners | `mcb.bot.use` |
| `/botteamally addowners <player1> [player2 ...]` | Add owners to an active TEAM_ALLY bot | `mcb.bot.use` |
| `/botteamally removeowners <player1> [player2 ...]` | Remove owners from an active TEAM_ALLY bot | `mcb.bot.use` |
| `/mcbreload <config|bot|maps|all>` | Reload config, despawn bots, clear cached options, or do all three | `mcb.admin.use` |

## Permissions
| Permission | Default | Description |
| --- | --- | --- |
| `mcb.bot.use` | `true` | Access normal, ally and team-ally workflows |
| `admin.host.bot` | `op` | Access event bot workflow |
| `mcb.admin.use` | `op` | Access maintenance and reload workflow |
...
| --- | --- |
| `com.monkey.mcbot.bot` | bot lifecycle, options, registry, spawn/update lookup |
| `com.monkey.mcbot.bot.ai` | high-level AI composition |
| `com.monkey.mcbot.bot.ai.controllers.*` | specialized combat and utility controllers |
| `com.monkey.mcbot.commands` | commands and tab completion |
| `com.monkey.mcbot.gui` | InvUI-based GUI construction |
| `com.monkey.mcbot.integration.api` | adapters from runtime core to public API |
| `com.monkey.mcbot.listener` | join, quit, world, death and tag listeners |
| `com.monkey.mcbot.placeholders` | PlaceholderAPI integration |
| `com.monkey.mcbot.nms` | bridge abstraction and runtime bridge selection |
| `com.monkey.mcbot.logging` | startup diagnostics and structured logging |
...
3. fires `MinecraftBotReadyEvent`

Third-party integrations should consume the public API from `mcbot-api`, not internal core classes.
