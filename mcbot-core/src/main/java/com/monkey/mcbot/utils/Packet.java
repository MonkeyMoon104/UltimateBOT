...
public final class Packet {
    ...
    public static void sendAddPlayerPacket(Player viewer, ITrainingBot bot) {
        ...
        ClientboundPlayerInfoUpdatePacket packet = NMSBridgeManager.get()
                .createAddPlayerPacket(
                        bot.asPlayer().getUUID(),
                        profile,
                        NMSBridgeManager.get().getProfileName(profile));
    ...
    public static void sendSpawnPlayerPacket(Player viewer, ITrainingBot bot) {
        ClientboundAddEntityPacket spawnPacket = NMSBridgeManager.get()
                .createSpawnPlayerPacket(
                        ...
                        bot.asPlayer().getY(),
                        bot.asPlayer().getZ(),
                        bot.asPlayer().getXRot(),
                        bot.asPlayer().getYRot(),
                        bot.asPlayer().getYHeadRot());
        ...
        handle.connection.send(new ClientboundMoveEntityPacket.Rot(
                bot.asPlayer().getId(), yBodyRot, xRot, bot.asPlayer().onGround()));

        List<SynchedEntityData.DataValue<?>> metadata =
                bot.asPlayer().getEntityData().getNonDefaultValues();
        if (metadata != null && !metadata.isEmpty()) {
            handle.connection.send(
                    new ClientboundSetEntityDataPacket(bot.asPlayer().getId(), metadata));
        }
    }
