package com.monkey.mcbot.event;

import com.monkey.mcbot.MinecraftBot;
import com.monkey.mcbot.api.event.BotEventSource;
import com.monkey.mcbot.api.event.BotSettingKey;
import com.monkey.mcbot.api.event.BotSettingsChangeEvent;
import com.monkey.mcbot.api.model.BotSnapshot;
import com.monkey.mcbot.bot.ai.ITrainingBot;

import java.util.Optional;
import java.util.UUID;
import java.util.Objects;

/** Shared typed gate used by every runtime setting entry point. */
public final class BotSettingEvents {
    private BotSettingEvents() { }

    public static <T> Optional<T> propose(MinecraftBot plugin, UUID ownerUUID, BotEventSource source,
                                           BotSettingKey key, T oldValue, T newValue, Class<T> valueType) {
        if (Objects.equals(oldValue, newValue)) return Optional.ofNullable(newValue);
        ITrainingBot bot = plugin.getBotRegistry().getBot(ownerUUID);
        BotSnapshot snapshot = plugin.getBotEventDispatcher().snapshot(ownerUUID, bot);
        if (snapshot == null) return Optional.empty();
        BotSettingsChangeEvent event = plugin.getBotEventDispatcher().publish(new BotSettingsChangeEvent(
                plugin.getBotEventDispatcher().nextSequence(snapshot.botUUID()), snapshot,
                source, key, oldValue, newValue));
        if (event.isCancelled() || !valueType.isInstance(event.getNewValue())) return Optional.empty();
        return Optional.of(valueType.cast(event.getNewValue()));
    }
}
