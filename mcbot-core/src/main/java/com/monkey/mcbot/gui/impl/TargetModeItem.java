...
public class TargetModeItem extends AbstractItem {
    ...
    public ItemProvider getItemProvider() {
        ...
        return new ItemBuilder(material(mode))
                .setDisplayName(ChatColorUtils.translate(plugin.getLangString("gui.target-mode-button.name", "&bAttack mode")))
                .addLoreLines(ChatColorUtils.translate(plugin.getLangString("gui.target-mode-button.current", "&7Current: &e%mode%")
                        .replace("%mode%", label(mode))))
                .addLoreLines(ChatColorUtils.translate(plugin.getLangString("gui.target-mode-button.click", "&aClick to change")));
    }
