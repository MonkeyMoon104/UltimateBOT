...
/** Reconnecting Server-Sent Events client for UltimateBot remote events. */
public final class UltimateBotEventBus implements AutoCloseable {
    private static final System.Logger LOGGER = System.getLogger(UltimateBotEventBus.class.getName());
    ...
    UltimateBotEventBus(URI baseUri, String token, HttpClient client, ObjectMapper mapper) {
    ...
    private final class Subscription implements BotEventSubscription {
        ...
        private void consume() throws Exception {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                response.body().close();
                throw new UltimateBotClientException("Event stream returned HTTP " + response.statusCode());
            }
            try (BufferedReader reader =
                ...
                while (active.get() && (line = reader.readLine()) != null) {
                    ...
                    try {
                        ...
                        LOGGER.log(System.Logger.Level.WARNING, "UltimateBot event listener failed", listenerError);
                    }
                }
            }
        }
